...
public class PassengerControllerUnitTest {
    ...
    public void testRegisterAndSendPassengerEvent() {
        ...
        verify(passengerService, times(2)).registerPassenger(testPassengerDto);
        verify(passengerEventProducer, times(1)).sendPassengerEvent("1");
    }

    @Test
    public void testGetPassengerByEmail() {
        when(passengerService.findPassengerByEmail(testEmail)).thenReturn(Optional.of(testPassengerDto));

        ResponseEntity<PassengerDto> response = passengerController.getPassengerByEmail(testEmail);

        assertEquals(200, response.getStatusCodeValue());
        assertEquals(testPassengerDto, response.getBody());
